/* Game of life */
/* Implementation using local variables only */

#include <stdio.h>
#include <stdlib.h>

void load_world_from_file(char *filename, int *worldX, int *worldY, char ***world, char ***next_world);
void save_world_to_file(char *filename, int worldX, int worldY, char **world);
void next_generation(int worldX, int worldY, char ***world, char ***next_world);
int get_next_state(int x, int y, int worldX, int worldY, char **world);
int count_neighbors(int x, int y, int worldX, int worldY, char **world);
void unload_world(int worldX, char **word, char **next_world);
...
int main(int argc, char **argv) {
    ...
    load_world_from_file(argv[1], &worldX, &worldY, &world, &next_world);

    /* advance world by 50 generations */
    for (i = 0; i < 50; i++) {
        next_generation(worldX, worldY, &world, &next_world);
    }

    save_world_to_file(argv[2], worldX, worldY, world);

    unload_world(worldX, world, next_world);
...
void next_generation(int worldX, int worldY, char ***world, char ***next_world) {
    ...
    for (i = 1; i < worldX + 1; i++) {
        for (j = 1; j < worldY + 1; j++) {
            (*next_world)[i][j] = get_next_state(i, j, worldX, worldY, *world);
        }
    }
    world_tmp = *world;
    *world = *next_world;
 ...
 */
int get_next_state(int x, int y, int worldX, int worldY, char **world) {
    int neightbours = count_neighbors(x, y, worldX, worldY, world);
 ...
 */
void unload_world(int worldX, char **world, char **next_world) {
    int i;
